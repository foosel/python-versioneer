...
| `pep440-tag`   | `TAG[.postDISTANCE.dev0#gSHORTHASH]`, a PEP-440 compatible version string which only changes from tagged version if the tree is dirty. Helpful if not all changes to the repository (e.g. Github metadata updates or Travis buildfiles) should cause new version numbers to be generated. |
